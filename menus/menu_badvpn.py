...
"""
Menu de gerenciamento para o BadVPN.

Esta versão ajustada evita recompilar o BadVPN toda vez que a porta é alterada.
Quando o serviço já está instalado, o menu chama a função `update_port` do
`BadVPNManager`, que edita o arquivo de serviço systemd e reinicia o daemon.
Caso o serviço não esteja instalado, o script de instalação original é
...
class BadVPNManager:
    ...
    def get_status(self):
        """Retorna status formatado do BadVPN e porta configurada."""
        ...
        try:
            ...
            status = (
                f"{MC.GREEN_GRADIENT}{Icons.ACTIVE} Ativo{MC.RESET}"
                if result.stdout.strip() == "active"
                else f"{MC.RED_GRADIENT}{Icons.INACTIVE} Inativo{MC.RESET}"
            )
            port = "7300"
    ...
    def get_bbr_status(self):
        """Retorna status do BBR (algoritmo de congestionamento)."""
    ...
    def update_port(self, port: str):
        """Atualiza a porta do BadVPN sem reinstalar.

        Abre o arquivo de serviço systemd, substitui a porta no parâmetro
        `--listen-addr` e reinicia o serviço. Retorna `(True, mensagem)`
        em caso de sucesso ou `(False, mensagem de erro)` caso contrário.
        """
        if not self.is_installed():
            return False, "BadVPN não está instalado."
        try:
            content = self.service_file.read_text()
            # substitui a porta em --listen-addr (0.0.0.0:PORTA)
            new_content, count = re.subn(r'--listen-addr\s+[^:]+:\d+', f'--listen-addr 0.0.0.0:{port}', content)
            if count == 0:
                return False, "Parâmetro --listen-addr não encontrado no arquivo de serviço."
            self.service_file.write_text(new_content)
            subprocess.run(['systemctl', 'daemon-reload'], check=True)
            subprocess.run(['systemctl', 'restart', 'badvpn-udpgw'], check=True)
            return True, ""
        except Exception as e:
            return False, str(e)
...
# ==================== FRAMES ====================
def build_main_frame(manager: BadVPNManager, status_msg: str = "") -> str:
...
def build_bbr_frame(status_msg: str = "") -> str:
...
def build_operation_frame(operation: str, port: str | None = None) -> str:
    """Frame para operações de instalação, alteração de porta e remoção."""
    s = []
                ...
                if port.isdigit() and 1 <= int(port) <= 65535:
                    ...
                    try:
                        if not manager.is_installed():
                            # Chama o script de instalação original
                            subprocess.run(['bash', str(manager.install_script), port], check=True)
                            status = f"BadVPN instalado e porta configurada: {port}"
                        else:
                            # Altera apenas a porta
                            success, msg = manager.update_port(port)
                            if success:
                                status = f"Porta alterada para {port}"
                            else:
                                status = f"Erro ao alterar porta: {msg}"
                    except Exception as e:

...
update_system() {
    ...
    apt update -qq || error_exit "Falha ao atualizar lista de pacotes."
}
...
install_basic_dependencies() {
    ...
    for package in "${packages[@]}"; do
        if ! dpkg -l | grep -q "^ii  $package "; then
            ...
            apt install -y "$package" || warning_msg "Falha ao instalar $package"
        fi
    done
}
...
install_rusty_socks5_proxy() {
    ...
    cp "$SCRIPT_DIR/main.rs" "$PROJECT_DIR/src/" || error_exit "Falha ao copiar main.rs."
    cp "$SCRIPT_DIR/Cargo.toml" "$PROJECT_DIR/src/" || error_exit "Falha ao copiar Cargo.toml."
    cp "$SCRIPT_DIR/rusty_socks_proxy.service" "$PROJECT_DIR/" || error_exit "Falha ao copiar arquivo de serviço."
    ...
    mv target/release/rusty_socks_proxy "$PROJECT_DIR/" || error_exit "Falha ao mover executável."
    chmod +x "$PROJECT_DIR/rusty_socks_proxy"
    
    # Configurar serviço systemd
    info_msg "Configurando serviço systemd..."
    cp "$PROJECT_DIR/rusty_socks_proxy.service" /etc/systemd/system/ || error_exit "Falha ao copiar arquivo de serviço."
    
    # Atualizar configurações no arquivo de serviço
    sed -i "s|Environment=\"SOCKS5_PORT=.*\"|Environment=\"SOCKS5_PORT=$socks5_port\"|g" /etc/systemd/system/rusty_socks_proxy.service
    
    # Recarregar systemd e iniciar serviço
    systemctl daemon-reload || error_exit "Falha ao recarregar daemon systemd."
    systemctl enable rusty_socks_proxy || error_exit "Falha ao habilitar serviço."
    systemctl start rusty_socks_proxy || error_exit "Falha ao iniciar serviço."
...
install_ssh_user_manager() {
    ...
    mkdir -p "$(dirname "$SSH_USER_MANAGEMENT_SCRIPT")"
    
    # Copiar script de gerenciamento
    if [[ -f "$SCRIPT_DIR/new_ssh_user_management.sh" ]]; then
        cp "$SCRIPT_DIR/new_ssh_user_management.sh" "$SSH_USER_MANAGEMENT_SCRIPT" || error_exit "Falha ao copiar script de gerenciamento SSH."
    else
        error_exit "Arquivo new_ssh_user_management.sh não encontrado."
    fi
    
    chmod +x "$SSH_USER_MANAGEMENT_SCRIPT" || error_exit "Falha ao dar permissão de execução."
...
manage_ssh_users_menu() {
    ...
    source "$SSH_USER_MANAGEMENT_SCRIPT"
...
install_dtproxy() {
    ...
    mkdir -p "$DTPROXY_DIR" || error_exit "Falha ao criar diretório $DTPROXY_DIR."
    
    # Copiar arquivos
    cp "$dtproxy_source_dir/dtproxy_x86_64" "$DTPROXY_DIR/" || error_exit "Falha ao copiar dtproxy_x86_64."
    cp "$dtproxy_source_dir/dtproxy_menu.sh" "$DTPROXY_DIR/" || error_exit "Falha ao copiar dtproxy_menu.sh."
    cp "$dtproxy_source_dir/dtproxy_port_menu.sh" "$DTPROXY_DIR/" || error_exit "Falha ao copiar dtproxy_port_menu.sh."
    
    # Dar permissões de execução
    chmod +x "$DTPROXY_DIR/dtproxy_x86_64" || error_exit "Falha ao dar permissão de execução."
    chmod +x "$DTPROXY_DIR/dtproxy_menu.sh"
    chmod +x "$DTPROXY_DIR/dtproxy_port_menu.sh"

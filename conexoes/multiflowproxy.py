...
async def handle_client(
    ...
    # === HANDSHAKE alinhado ao Rust: 101 imediato -> probe (leitura com timeout, usada para parse/probe e forwardada) -> status adicionais ===
    ...
    # 2) Probe com timeout para inspecionar dados reais (leitura, usada para parse/probe, forward depois) - sem descarte inicial para evitar perda
    try:
        probe_data = await asyncio.wait_for(client_reader.read(8192), timeout=1.0)
    except asyncio.TimeoutError:
        probe_data = b""
    except Exception as exc:
        logging.error("Falha ao ler dados probe: %s", exc)
        client_writer.close()
        await client_writer.wait_closed()
        return
    ...
    # 4) Envio dos demais status após o probe:
    ...
    # 5) Determina backend por cabeçalho preferencial; fallback para heurística no probe_data
    backend_host: str
    backend_port: int
    ...
    # 7) Forward dos dados probe para o backend (garante que dados inspecionados sejam enviados, sem perda)
    if probe_data:

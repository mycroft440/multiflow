...
async def transfer_data(source_reader, dest_writer):
    try:
        ...
        print(f"Erro no transfer_data: {str(e)}")
    # Removido close() para evitar fechamento prematuro; deixar o gather/drop fechar
...
# === Funções de Gerenciamento (Instalação, Serviços) ===
...
def del_proxy_port(port):
    # Check se porta existe em PORTS_FILE
    if not os.path.exists(PORTS_FILE) or str(port) not in open(PORTS_FILE).read():
        print(f"Porta {port} não encontrada.")
        return
    try:
...
def restart_proxy_port(port):
    # Check se porta existe
    if not os.path.exists(f"/etc/systemd/system/proxy{port}.service"):
        print(f"Serviço para porta {port} não encontrado.")
        return
    try:
...
def show_menu():
    while True:
        ...
        if option == '1':
            ...
            status = input("\n➜ Digite o status de conexão (deixe vazio para 'Switching Protocols'): ") or "Switching Protocols"

...
def parse_headers(text: str, data: bytes) -> Dict[str, str]:
...
async def handle_client(
    ...
    # 2) Leitura inicial curta e descarte (alinhado ao Rust: lê 1024 bytes e descarta, para consumir request falso)
    try:
    ...
    # 3) Parse geral de headers a partir da leitura inicial (mantido para features extras, mas opcional)
    header_text = initial_data.decode("utf-8", errors="ignore")
    headers = parse_headers(header_text, initial_data)
    ...
    # 4) Envio dos demais status após a leitura inicial (alinhado ao 200 no Rust)
    try:
        ...
        for code in sorted(enabled):
            if code in (101,):  # 101 já foi enviado
                continue
            if code in to_send:
                continue
            to_send.append(code)
    ...
    # 5) Probe com timeout para inspecionar dados reais (alinhado ao peek: lê e forwarda depois)
    try:
    ...
    # 6) Determina backend pela heurística no probe_data (force local como Rust, ignore headers para alinhamento)
    backend_host, backend_port = await probe_backend_from_data(probe_data)  # Sempre local
    ...
    # 8) Forward dos dados probe para o backend (alinhado ao peek não-consumidor)
    if probe_data:

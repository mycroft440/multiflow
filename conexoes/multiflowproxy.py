...
class ConfigManager:
    ...
    def save_config(self):
        try:
            ...
            print(f"\033[91mErro ao salvar configuração: {e}\033[0m")
            return False
...
def generate_ssl_cert(domain, email):
    try:
        ...
        print(f"\n\033[91mErro ao gerar certificado SSL: {e}\033[0m")
        return False
...
def configure_ssl(config_manager):
    ...
    while not domain:
        print("\033[91mDomínio obrigatório.\033[0m")
    ...
    if generate_ssl_cert(domain, email):
        ...
        print("\033[91mFalha ao configurar SSL.\033[0m")

def remove_ssl(config_manager):
    if not config_manager.config['ssl']['enabled']:
        print("\n\033[91mSSL já está desativado.\033[0m")
        return
...
def show_menu():
    ...
    while True:
        os.system('clear')
        print("\033[95mMULTIFLOW PROXY\033[0m")
        ...
        print(f"Portas: \033[92m{active_ports}\033[0m")
        
        ssl_status = "\033[92mAtivado\033[0m" if config_manager.config['ssl']['enabled'] else "\033[91mDesativado\033[0m"
        print(f"SSL: {ssl_status}\n")
        
        print("\033[94m---------------------------\033[0m")
        print("\033[95mMENU\033[0m")
        print("\033[94m---------------------------\033[0m")
        
        if not is_proxy_installed():
            print("\033[96m[1]\033[0m Instalar Proxy")
            print("\033[96m[0]\033[0m Sair")
        else:
            print("\033[96m[1]\033[0m Adicionar Porta")
            print("\033[96m[2]\033[0m Remover Porta")
            print("\033[96m[3]\033[0m Reiniciar Porta")
            print("\033[96m[4]\033[0m Desinstalar Proxy")
            print("\033[96m[5]\033[0m Alternar Traffic Shaping")
            print("\033[96m[6]\033[0m Adicionar SSL")
            print("\033[96m[7]\033[0m Remover SSL")
            print("\033[96m[0]\033[0m Sair")
        
        print("\n\033[94m---------------------------\033[0m")
        option = input("\033[96mEscolha uma opção: \033[0m")
        
        if option == '1':
            if not is_proxy_installed():
                install_proxy()
                port_input = input("\n\033[96mDeseja iniciar proxy em qual porta? \033[0m")
                while not port_input.isdigit() or int(port_input) < 1 or int(port_input) > 65535:
                    print("\033[91mDigite uma porta válida (1-65535).\033[0m")
                    port_input = input("\033[96mDeseja iniciar proxy em qual porta? \033[0m")
                add_proxy_port(int(port_input))
            else:
                port = input("\n\033[96m > Digite a porta para adicionar: \033[0m")
                while not port.isdigit() or int(port) < 1 or int(port) > 65535:
                    print("\033[91mDigite uma porta válida (1-65535).\033[0m")
                    port = input("\033[96m > Digite a porta: \033[0m")
                add_proxy_port(int(port))
            input("\n\033[96mPressione Enter para continuar...\033[0m")
           
        elif option == '2' and is_proxy_installed():
            port_info = list_active_ports()
            if port_info:
                print("\n\033[96mPortas ativas:\033[0m")
                for port, status in port_info:
                    print(f" \033[92m{port}\033[0m - {status}")
                port = input("\n\033[96m > Digite a porta para remover: \033[0m")
                while not port.isdigit():
                    print("\033[91mDigite uma porta válida.\033[0m")
                    port = input("\033[96m > Digite a porta: \033[0m")
                del_proxy_port(int(port))
            else:
                print("\033[91mNenhuma porta ativa para remover.\033[0m")
            input("\n\033[96mPressione Enter para continuar...\033[0m")
           
        elif option == '3' and is_proxy_installed():
            port_info = list_active_ports()
            if port_info:
                print("\n\033[96mPortas disponíveis para reiniciar:\033[0m")
                for port, status in port_info:
                    print(f" \033[92m{port}\033[0m - {status}")
                port = input("\n\033[96m > Digite a porta para reiniciar (ou 'all' para todas): \033[0m")
                ...
                print("\033[91mNenhuma porta ativa para reiniciar.\033[0m")
            input("\n\033[96mPressione Enter para continuar...\033[0m")
            ...
            status = "\033[92mativada\033[0m" if enabled else "\033[91mdesativada\033[0m"
            ...
            input("\033[96mPressione Enter para continuar...\033[0m")

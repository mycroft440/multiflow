...
import time
import base64
import random  # Adicionado para randomização de chunks (ideia inovadora da web para DPI bypass)
...
class ConnectionHandler(threading.Thread):
                    ...
                    try:
                        ...
                        if data:
                            if in_ is self.target:
                                # Enviar chunked para bypass
                                self.send_chunked(self.client, data)
                            else:
    ...
    def send_chunked(self, sock, data):
        """Envio chunked para fragmentar e bypass DPI"""
        if not isinstance(data, bytes):
            data = data.encode()
        # Sugestão inovadora: Randomize tamanho de chunks para evitar padrões DPI (baseado em POKUXdpi)
        if len(data) > 1024:  # Apenas para dados grandes
            chunks = [data[i:i + random.randint(512, 2048)] for i in range(0, len(data), random.randint(512, 2048))]
        else:
            chunks = [data]
        for chunk in chunks:
            chunk_size = hex(len(chunk))[2:].encode()
            try:
                sock.sendall(chunk_size + b'\r\n' + chunk + b'\r\n')
            except socket.error:
                # Ignora erros se o socket já foi fechado
                pass
        # Envie chunk final zero se necessário (boa prática para encerrar stream)
        sock.sendall(b'0\r\n\r\n')

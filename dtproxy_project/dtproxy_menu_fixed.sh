...
install_dtproxy() {
    ...
    local dtproxy_source_dir="/home/ubuntu/multiflow/multiflow-main/dtproxy_project"
...
install_socks5() {
    ...
    if [[ -f "$SOCKS5_EXEC" ]]; then
        echo -e "${YELLOW}[AVISO]${NC} SOCKS5 já parece estar instalado."
        read -p "Pressione Enter para continuar..." -n 1
        return
    fi
    
    echo -e "${YELLOW}[INFO]${NC} Compilando o projeto Rust para SOCKS5..."
    (cd /home/ubuntu/multiflow/multiflow-main && cargo build --release)
    
    if [[ $? -ne 0 ]]; then
        echo -e "${RED}[ERRO]${NC} Falha ao compilar o projeto Rust."
        read -p "Pressione Enter para continuar..." -n 1
        return
    fi
    
    echo -e "${YELLOW}[INFO]${NC} Criando diretório de instalação: $SOCKS5_DIR..."
    mkdir -p "$SOCKS5_DIR"
    
    echo -e "${YELLOW}[INFO]${NC} Copiando executável para $SOCKS5_DIR..."
    cp /home/ubuntu/multiflow/multiflow-main/target/release/rusty_socks_proxy "$SOCKS5_EXEC"
    
    echo -e "${YELLOW}[INFO]${NC} Copiando arquivo de serviço systemd..."
    cp /home/ubuntu/multiflow/multiflow-main/rusty_socks_proxy.service "$SOCKS5_SERVICE_FILE"
    
    echo -e "${YELLOW}[INFO]${NC} Recarregando daemon systemd, habilitando e iniciando serviço SOCKS5..."
    systemctl daemon-reload
    systemctl enable rusty_socks_proxy
    systemctl start rusty_socks_proxy
    
    sleep 2
    
    if systemctl is-active --quiet rusty_socks_proxy; then
        echo -e "${GREEN}[SUCESSO]${NC} SOCKS5 instalado e iniciado com sucesso na porta 1080."
    else
        echo -e "${RED}[ERRO]${NC} Falha ao iniciar o serviço SOCKS5. Verifique os logs com \'journalctl -u rusty_socks_proxy\'."
    fi
...
alter_socks5_port() {
    ...
    if [[ ! -f "$SOCKS5_SERVICE_FILE" ]]; then
        echo -e "${RED}[ERRO]${NC} Serviço SOCKS5 não encontrado. Instale-o primeiro."
        read -p "Pressione Enter para continuar..." -n 1
        return
    fi
    
    current_port=$(grep -oP \'Environment="SOCKS5_PORT=\\K[0-9]+\' "$SOCKS5_SERVICE_FILE")
    echo -e "${YELLOW}[INFO]${NC} Porta atual do SOCKS5: ${current_port:-1080}"
    ...
    echo -e "${YELLOW}[INFO]${NC} Parando serviço SOCKS5..."
    systemctl stop rusty_socks_proxy
    
    echo -e "${YELLOW}[INFO]${NC} Alterando porta no arquivo de serviço..."
    sudo sed -i "s/^Environment=\"SOCKS5_PORT=[0-9]*\"/Environment=\"SOCKS5_PORT=$new_port\"/" "$SOCKS5_SERVICE_FILE"
    
    echo -e "${YELLOW}[INFO]${NC} Recarregando daemon systemd e iniciando serviço SOCKS5 com a nova porta..."
    systemctl daemon-reload
    systemctl start rusty_socks_proxy
    ...
    if systemctl is-active --quiet rusty_socks_proxy; then
        echo -e "${GREEN}[SUCESSO]${NC} SOCKS5 reiniciado com sucesso na porta $new_port."
    else
        echo -e "${RED}[ERRO]${NC} Falha ao reiniciar o serviço SOCKS5 na porta $new_port. Verifique os logs."
    fi
...
open_socks5_port() {
    ...
    echo -e "${YELLOW}[INFO]${NC} Verificando se o serviço SOCKS5 está rodando..."
    if systemctl is-active --quiet rusty_socks_proxy; then
        echo -e "${GREEN}[SUCESSO]${NC} Serviço SOCKS5 está ativo na porta $port."
    else
        echo -e "${YELLOW}[AVISO]${NC} Serviço SOCKS5 não está ativo. Iniciando..."
        systemctl start rusty_socks_proxy
        if systemctl is-active --quiet rusty_socks_proxy; then
            echo -e "${GREEN}[SUCESSO]${NC} Serviço SOCKS5 iniciado na porta $port."
        else
            echo -e "${RED}[ERRO]${NC} Falha ao iniciar o serviço SOCKS5."
        fi
    fi
...
remove_socks5() {
    ...
    echo -e "${YELLOW}[INFO]${NC} Parando serviço SOCKS5..."
    systemctl stop rusty_socks_proxy 2>/dev/null || true
    
    echo -e "${YELLOW}[INFO]${NC} Desabilitando serviço SOCKS5..."
    systemctl disable rusty_socks_proxy 2>/dev/null || true
    
    echo -e "${YELLOW}[INFO]${NC} Removendo arquivo de serviço systemd..."
    rm -f "$SOCKS5_SERVICE_FILE"
    
    echo -e "${YELLOW}[INFO]${NC} Recarregando daemon systemd..."
    systemctl daemon-reload
    
    echo -e "${YELLOW}[INFO]${NC} Removendo diretório $SOCKS5_DIR..."
    rm -rf "$SOCKS5_DIR"
    
    echo -e "${GREEN}[SUCESSO]${NC} SOCKS5 removido completamente."
    read -p "Pressione Enter para continuar..." -n 1
}

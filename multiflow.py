...
# Esta seção localiza a raiz do projeto e importa módulos necessários dinamicamente.
...
def _find_multiflow_root():
    ...
    env_home = os.environ.get("MULTIFLOW_HOME")  # Obtém variável de ambiente se definida
    if env_home:
    ...
    try:
        script_dir = os.path.dirname(os.path.realpath(__file__))  # Diretório atual do script
...
def _import_by_file_path(alias, filepath):
    ...
    try:
        spec = importlib.util.spec_from_file_location(alias, filepath)  # Cria especificação do módulo
        if spec and spec.loader:
            module = importlib.util.module_from_spec(spec)  # Cria módulo a partir da spec
...
def bootstrap_imports():
    ...
    missing = [alias for alias in targets.keys() if alias not in imported]  # Módulos faltando
    if missing and root:
    ...
    still_missing = [alias for alias in targets.keys() if alias not in imported]  # Módulos ainda faltando
    if still_missing:
    ...
    globals().update(imported)  # Atualiza o escopo global com módulos importados
...
from ferramentas import manusear_usuarios  # noqa: F401  (já no globals) - Ignora linting
...
# Classe para gerenciar o terminal, incluindo tela alternativa e renderização.
...
class TerminalManager:
    ...
    @staticmethod
    def size():
        ts = shutil.get_terminal_size(fallback=(80, 24))  # Obtém tamanho do terminal
...
def modern_header():
    ...
    s.extend(["  " + l + "\n" for l in logo_lines])  # Adiciona logo com indentação
...
def modern_box(title, content_lines, icon="", primary=MC.CYAN_GRADIENT, secondary=MC.CYAN_LIGHT):
    ...
    title_text = f" {icon}{title} " if icon else f" {title} "  # Texto do título com ícone
    ...
    for line in content_lines:
        clean = re.sub(r'\033\[[0-9;]*m', '', line)  # Remove códigos de cor para cálculo de comprimento
...
def menu_option(number, text, icon="", color=MC.CYAN_GRADIENT, badge=""):
    num = f"{color}{MC.BOLD}[{number}]{MC.RESET}" if number != "0" else f"{MC.RED_GRADIENT}{MC.BOLD}[0]{MC.RESET}"  # Número colorido
    b = f" {MC.PURPLE_GRADIENT}{MC.WHITE}{MC.BOLD} {badge} {MC.RESET}" if badge else ""  # Badge se presente
    return f"  {num} {icon}{MC.WHITE}{text}{b}{MC.RESET}\n"  # Retorna string da opção
...
def progress_bar(percent, width=18):
    ...
    return f"[{c}{'█' * filled}{MC.DARK_GRAY}{'░' * empty}{MC.RESET}] {c}{percent:5.1f}%{MC.RESET}"  # Retorna barra
...
def monitorar_uso_recursos(intervalo_cpu=0.10):
    try:
        ...
        cpu_percent = psutil.cpu_percent(interval=intervalo_cpu)  # Obtém uso de CPU
        return {'ram_percent': ram.percent, 'cpu_percent': cpu_percent}  # Retorna dicionário
    except Exception:
        return {'ram_percent': 0, 'cpu_percent': 0}  # Retorna zeros em erro

# Função para obter info do sistema
def get_system_info():
    info = {"os_name": "Desconhecido", "ram_percent": 0, "cpu_percent": 0}  # Info padrão
    try:
...
def system_panel_box():
    ...
    if services:
        ...
        if len(services) > 4:
            content.append(" " * 13 + " │ ".join(services[4:8]))  # Segunda linha se necessário
    else:
...
def build_connections_frame(status_msg=""):
    ...
    s.append("\n")
    s.append(modern_box("GERENCIAR CONEXÕES", [], Icons.NETWORK, MC.CYAN_GRADIENT, MC.CYAN_LIGHT))  # Caixa
    s.append("\n")
...
def build_tools_frame(status_msg=""):
    ...
    s.append(system_panel_box())  # Painel
    s.append("\n")
    s.append(modern_box("FERRAMENTAS DE OTIMIZAÇÃO", [], Icons.TOOLS, MC.ORANGE_GRADIENT, MC.ORANGE_LIGHT))  # Caixa
    s.append("\n")
...
def conexoes_menu():
    ...
    while True:
        ...
        TerminalManager.before_input()  # Prepara input
        choice = input(f"\n{MC.PURPLE_GRADIENT}{MC.BOLD}└─ Escolha uma opção: {MC.RESET}").strip()  # Lê escolha
        TerminalManager.after_input()  # Após input
...
def ferramentas_menu():
    ...
    while True:
        ...
        choice = input(f"\n{MC.PURPLE_GRADIENT}{MC.BOLD}└─ Escolha uma opção: {MC.RESET}").strip()  # Lê
...
def atualizar_multiflow():
    ...
    TerminalManager.before_input()  # Prepara
    confirm = input(f"\n{MC.BOLD}{MC.WHITE}Deseja continuar com a atualização? (s/n): {MC.RESET}").strip().lower()  # Confirma
    TerminalManager.after_input()  # Após

    if confirm == 's':
        try:
            script_dir = os.path.dirname(os.path.realpath(__file__))  # Diretório
    ...
    while True:
        try:
            ...
            if choice == "1":
                ...
                status = "Opção inválida. Pressione 1-7 ou 0 para sair."  # Erro

        except KeyboardInterrupt:
            TerminalManager.render(build_main_frame("Interrompido pelo usuário."))  # Interrupção
            time.sleep(0.5)
            break
        except Exception as e:
